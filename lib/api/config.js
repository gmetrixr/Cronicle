	...
	api_config: function(args, callback) {
		...
		var resp = {
			...
			config: Tools.mergeHashes( this.server.config.get('client'), {
				...
				web_socket_hostname: process.env.IS_LOCAL ? undefined : this.server.config.get('web_socket_hostname') || undefined,
				custom_live_log_socket_url: process.env.IS_LOCAL ? undefined : this.server.config.get('custom_live_log_socket_url') || undefined,
